...
class ConstructionList(scrapy.Spider):
    ...
    def parse_list_category(self, response):
        ...
        for i, j in zip(nexts, next_cat):
            if i == 'Attachments' or 'Compaction Equipment' or 'Power System Generation':
                ...
                yield scrapy.Request(next1, self.used_attachment_item,meta={'category':item['category']})
            else:
                next1 = response.urljoin(j)
                item['category'] = {'cat1_name': i,
                                    'cat2_name': '',
                ...
                yield scrapy.Request(next1, self.parse_item_list)
    ...
    def used_attachment_item(self, response):
        ...
        for i in used_attachment:
            ...
            cat_temp['cat2_name'] = response.xpath("//div/ul/li[@class='category']/a/@text()").get()
            used_attachment_path = response.urljoin(i)
            yield scrapy.Request(used_attachment_path, self.parse_item_list)
    ...
    def parse_item_list(self, response):
        ...
        for construct in collection_items:
            ...
            item['item_url']   = construct.xpath("//div[@class='machine-model']/a/@href").get()
        ...
        if next_page is not None:
            ...
            yield scrapy.Request(next1, callback=self.parse_item_list, dont_filter=True)
        yield item
